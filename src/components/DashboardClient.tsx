...
function DashboardClient({ ownerId }: { ownerId: string }) {
    ...
    useEffect(() => {
        if (!ownerId) {
            console.warn("ownerId is missing — user may not be logged in properly.")
            return
        }
        ...
        <div className='min-h-screen bg-zinc-50 text-zinc-900'>
            ...
            >
                <div className='max-w-7xl mx-auto px-6 h-16 flex items-center justify-between'>
                    <div className='text-lg font-semibold tracking-tight' onClick={() => navigate.push("/")} style={{ cursor: 'pointer' }}>Support <span className='text-zinc-400'>AI</span></div>
            ...
            <div className='flex justify-center px-4 py-14 mt-20'>
                ...
                >
                    ...
                    {!ownerId && (
                        <div className='mb-6 px-4 py-3 rounded-xl bg-red-50 text-red-600 text-sm border border-red-200'>
                            ⚠ Session not found. Please <span className='underline cursor-pointer' onClick={() => navigate.push("/api/auth/login")}>log in</span> again.
                        </div>
                    )}
